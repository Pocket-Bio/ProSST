...
from prosst.structure.utils.data_utils import (
    BatchSampler,
    convert_graph,
...
def graph_conventer(
    ...
    def process_graph_file(graph_file, subgraph_depth, max_distance):
    ...
    if error_proteins:
        ...
        if error_file is None:
            error_file = os.path.join(
                os.path.dirname(pdb_files[0]),
                f"{os.path.basename(pdb_files[0]).split('.')[0]}_error.csv",
            )
...
class SSTPredictor:
    def __init__(
        ...
        if cluster_dir is None:
            ...
            self.cluster_model = (
                ...
                else [f"{structure_vocab_size}.joblib"]
            )
    ...
    def predict_from_subgraph(self, subgraph_dir, pdb_dir):
        ...
        data_loader, results = subgraph_conventer(
            subgraph_dir, pdb_dir, self.max_batch_nodes, self.num_processes
        )
